...
function processHumanoidTexture(creatureTexture: CreatureTextureConfig, itemConfig: ItemConfig): CreatureTextureConfig {
  ...
  const leftArm = creatureTexture.bodyPart[creatureDefaults.BodyPart.LeftArm];
  const rightArm = creatureTexture.bodyPart[creatureDefaults.BodyPart.RightArm];

  const colorIndex = itemConfig.texture.colorPaletteIndex[itemConfig.condition || ''];

  [].concat(torso, leftArm, rightArm).forEach(rect => {
